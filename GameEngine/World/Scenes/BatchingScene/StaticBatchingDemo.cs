...
public class StaticBatchingDemo : IWorldFactory
{
    ...
    {
        const int size = 10;
        Mesh[] cubes = new Mesh[size * size * size];
        ...
        for (int i = 0; i < size; i++)
        {
            for (int j = 0; j < size; j++)
            {
                for (int k = 0; k < size; k++)
                {
                    ...
                    cubes[i * size * size + j * size + k] = new CubeMeshData(1f, position).Build();
                }
            }
        }

        return StaticBatching.Concatenate(cubes);
    }
    ...
    private Mesh GetBigSphereMesh()
    {
        ...
        for (int i = 0; i < size; i++)
        {
            ...
            spheres[i] = new SphereMeshData(1f, 24, 24, new Vector3(x, y, z)).Build();
        }

        return StaticBatching.Concatenate(spheres);
    }
}